    ...
    */
    $(document).delegate(FloatTarget.StoreBoxTarget.StoreContentSpan, "click", function () {
        ...
        if (BookingInfo.StoreName.indexOf("免费") > -1) {
            ...
            selectStoreTimeInit($(this).attr("from_time"), $(this).attr("time"), BookingInfo.Type);
        }
    });
    /*
    *常用门店
...
$(function () {
    ...
    $(document).on('click', '.show-carlist .btn-book', function () {
        ...
        var ipcar = $(this).data("ipcar");
        var price = $(this).data("price");
        window.location.href = Url.Booking + "?carTypeId=" + carid + "&storeId=" + sid + "&priceType=" + priceType + "&groupId=" + gid + "&isipcar=" + ipcar + "&price=" + price;
    });
...
$(function () {
    ...
    $(document).on('click', '.video-total', function () {
        console.log($(this).data('cartypeids'));
        var ids = $(this).data('cartypeids');
        console.log(ids);
        $.ajax({
            type: 'post',
            url: BookingAjax.Url.GetCarTypeVideoInfo,
            data: { carTypeIds: ids },
            dataType: "html",
            async: true,
            cache: false,
            success: function (data) {
                window.myshowBox('', '', '800', '470', 'false', 'window-scroll');
                $(".div-myshowbox").html(data);
                ...
                $('.swiper-btn-prev').on('click', function (e) {
                    ...
                    var prevId = 'carVideo' + mySwiper.previousIndex;
                    var videoprev = document.getElementById(prevId);
                    videoprev.pause();
                });
    ...
    /**车型配置弹窗**/
    $(document).on('click', '#carDetail', function () {
        window.myshowBox('', '', '940', '600', 'false', 'window-scroll');
        var param = $(this).data("cid");
        window.BookingAjax.Post(BookingAjax.Url.cartypeConfigPath, param, 'cartyoeConfig', function (data) {
            $(".div-myshowbox").html(data);
        });

        $(document).on('click', '.video-li', function () {
            var ids = $(this).data('cartypeids');
            console.log(ids);
            $.ajax({
                type: 'post',
                url: BookingAjax.Url.CarTypeVideoInfo,
                data: { carTypeId: ids },
                dataType: "html",
                async: true,
                cache: false,
                success: function (data) {
                    console.log(data)
                    $('.current-carvideo').html(data);
                    $('.current-carvideo').show();
                }
            });
        });

        $(document).on('click', '#CloseVideo', function () {
            $('.current-carvideo').hide();
        });
    });
    ...
    //显示更多车型
    $(document).on('click', '.load-more-box a', function () { showMoreCars(); });
